...
app_workout_api = Blueprint('workout_api', __name__,
                         url_prefix='/workout_api',
                         template_folder='templates/workout/',
                         static_folder='static',
                         static_url_path='static')

# API generator https://flask-restful.readthedocs.io/en/latest/api.html#id1
api = Api(app_workout_api)


# Method #2 for CRUD
@app_workout_api.route('/')
def workout_api():
    """obtains all Users from table and loads Admin Form"""
    return render_template("workout_async.html", table=workouts_all())
...
class WorkoutsAPI:
    ...
    class _Create(Resource):
        def post(self, name, departingLocation, arrivalLocation, departingTime, arrivalTime):
            po = Workouts(name, departingLocation, arrivalLocation, departingTime, arrivalTime)
            person = po.create()
            if person:
                return person.read()
    ...
    class _ReadID(Resource):
        def get(self, workoutid):
            ...
            if po is None:
                return {'message': f"{workoutid} is not found"}, 210
            data = po.read()
            return data
    ...
    class _ReadILike(Resource):
        def get(self, term):
            return workouts_ilike(term)

    # class for update/put
    class _Update(Resource):
        def put(self, departingLocation, name):
            po = workout_by_departingLocation(departingLocation)
            if po is None:
...
def api_tester():
    ...
    url = 'http://localhost:5222/workout_api'
